...
uint8_t tx_buf[BUFFER_SIZE] = {0xCA};   // Response to master (khác với Slave 1)
...
void spiReceiveTask(void *pvParameters) {
  ...
  while (1) {
    ...
    if (digitalRead(HSPI_CS) == LOW) {
      Serial.println("Slave 2: CS pin LOW detected");
      ...
      if (ret == ESP_OK && trans.trans_len > 0) {
        Serial.print("Slave 2 received: 0x");
        ...
        switch (rx_buf[0]) {
          case 0x00:  // Off
            ...
            pixels.show();
            Serial.println("Slave 2 RGB: Off");
            break;
          case 0x01:  // Red
            pixels.setPixelColor(0, pixels.Color(255, 0, 0));
            pixels.show();
            Serial.println("Slave 2 RGB: Red");
            break;
          case 0x02:  // Green
            pixels.setPixelColor(0, pixels.Color(0, 255, 0));
            pixels.show();
            Serial.println("Slave 2 RGB: Green");
            break;
          case 0x03:  // Blue
            pixels.setPixelColor(0, pixels.Color(0, 0, 255));
            pixels.show();
            Serial.println("Slave 2 RGB: Blue");
            break;
        }
      } else {
        Serial.println("Slave 2: No data received or SPI error");
      }
    }
...
void setup() {
  ...
  Serial.println("Slave 2 SPI initialized");
